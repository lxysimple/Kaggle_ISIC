...
class ISICDataset_1(Dataset):
    def __init__(self, df, file_hdf, transforms=None):
        self.fp_hdf = h5py.File(file_hdf, mode="r")

        self.df = df[df["target"] == 1].reset_index()
        self.isic_ids = df['isic_id'].values
        self.targets = df['target'].values
        self.transforms = transforms

        print(file_hdf)
        print('just 1: ', len(self.df))
...
class ISICModel1(nn.Module):
    ...
    def __init__(self, model_name, pretrained=True, out_dim=1, n_meta_features=200, n_meta_dim=[3*512, 3*128, 3*32], checkpoint_path=None):
        ...
        self.myfc = nn.Linear(n_meta_dim[2]+16, out_dim)
...
# model = load_model('/root/autodl-tmp/Kaggle_ISIC/eva/AUROC0.5319_Loss0.3800_pAUC0.1340_fold1.bin')
